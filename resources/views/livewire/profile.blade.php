<div x-data="{}" class="max-w-3xl p-8">
    <h3 class="bg-blue-900 text-gray-100 py-2 px-4">Profile</h3>
    <div class="flex flex-wrap p-4  bg-blue-200">
        <div class="w-2/3 flex flex-wrap items-start">
            <form  @submit="$refs.submit.disabled = true" method="POST" action="{{route('profile.update')}}" class="w-full flex flex-wrap">
                @csrf
                ...
                <div class="w-full mt-2 pt-2">
                    <button x-ref="submit" class="px-4 py-1 bg-blue-800 hover:bg-blue-900 hover:text-blue-100 text-blue-100">Update</button>
                </div>
            </form>
        </div>
        <div class="w-1/3 border-2 border-gray-900 border-dashed h-48 flex items-center justify-center relative">
            <form x-ref="upload" action="{{route('profile-picture')}}" class="z-20 w-full h-full block" enctype="multipart/form-data" method="POST">
                @csrf
                <label for="file" class="w-full h-full block flex items-center justify-center text-blue-900 text-2xl">
                    @if(!$image)
                    Profile Picture
                    @endif
                    <input type="file" id="file" name="file" class="hidden" @change="$refs.submit.disabled = true; $refs.upload.submit()">
                </label>
            </form>
            @if($image)
